...
PRODUKTDETALJER (VIGTIGT — 3-5 SÆTNINGER, KUN FUNKTIONEL BESKRIVELSE):
- Skriv en beskrivende tekst på 3-5 sætninger om produktet på dansk
- ALDRIG styling tips, ALDRIG modetips, ALDRIG "style den med...", ALDRIG "perfekt til..."
- ALDRIG tillægsord som "elegant", "raffineret", "eksklusiv", "tidløs", "sofistikeret"
- ALDRIG subjektive vurderinger — KUN beskriv hvordan produktet FYSISK ser ud
- Beskriv KUN: ærmelængde (kort/lang), kravetype (rund hals/V-hals/skjortekrave), pasform (regular/slim/oversized/loose), lukning (knapper/lynlås/uden), lommer, længde, kanter, detaljer
...
- DÅRLIGT eksempel (ALDRIG skriv sådan): "Perfekt til en afslappet hverdagslook. Style den med sneakers for et casual look."
...
def push_product_to_shopify(
    ...
    # ── 4. Set metafields (gender, brand_collection, material, color, season) ──
    gender = product.get("gender", "").lower()
    gender_values = []
    if gender == "unisex":
    ...
    for mf in metafields:
        try:
            ...
            errors_log.append(f"Metafield ({mf['key']}): {e}")
