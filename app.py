...
st.markdown("""
<style>
/* ══ STRØM Import — Variation A Design System ══ */
...
:root {
    ...
    --strom-sunken: #f0f0f5;
    --strom-line: #e2e3e8;
    --strom-line-soft: #ededf2;
    --strom-fg: #1b2033;
    --strom-fg-soft: #5a5f72;
    --strom-fg-muted: #8e92a2;
    --strom-accent: #4a6fa5;
    --strom-accent-ink: #2e4e7a;
    --strom-accent-soft: #e8eef8;
    --strom-ok: #2c9960;
    --strom-ok-soft: #e2f5ec;
    --strom-warn: #c08830;
    --strom-warn-soft: #fdf5e8;
    --strom-err: #c03030;
    --strom-err-soft: #fbeaea;
    --strom-font-display: 'Inter Tight', ui-sans-serif, system-ui, sans-serif;
    --strom-font-ui: 'Inter', ui-sans-serif, system-ui, sans-serif;
    --strom-font-mono: 'JetBrains Mono', ui-monospace, monospace;
    --strom-radius: 10px;
    --strom-radius-sm: 6px;
}

.stApp {
    font-family: var(--strom-font-ui) !important;
    -webkit-font-smoothing: antialiased;
}

/* ── Headers ── */
h1 {
    font-family: var(--strom-font-display) !important;
    font-weight: 600 !important;
    letter-spacing: -0.03em !important;
    font-size: 1.75rem !important;
    color: var(--strom-fg) !important;
}
h2 {
    font-family: var(--strom-font-display) !important;
    font-weight: 600 !important;
...
[data-testid="stSidebar"] {
    background: var(--strom-elev) !important;
    border-right: 1px solid var(--strom-line) !important;
}
[data-testid="stSidebar"] [data-testid="stMarkdownContainer"] p,
[data-testid="stSidebar"] [data-testid="stMarkdownContainer"] span {
    color: var(--strom-fg-soft) !important;
    font-size: 0.85rem;
}
...
/* ── Sidebar wordmark ── */
.strom-wordmark {
    font-family: var(--strom-font-display);
    font-weight: 600;
    font-size: 13px;
    letter-spacing: 0.04em;
    color: var(--strom-fg);
    padding: 0 0 8px 0;
    border-bottom: 1px solid var(--strom-line-soft);
    margin-bottom: 12px;
}
.strom-wordmark .wm-dash { color: var(--strom-accent); margin: 0 4px; }
.strom-wordmark .wm-sub { font-weight: 400; color: var(--strom-fg-soft); }
.strom-wordmark .wm-tag {
    display: block;
    margin-top: 4px;
    font-size: 10px;
    font-family: var(--strom-font-mono);
    letter-spacing: 0.06em;
    color: var(--strom-fg-muted);
...
.status-row .sr-label { color: var(--strom-fg-soft); }
.status-row .sr-ok {
    display: inline-flex;
    align-items: center;
    gap: 6px;
...
.status-row .sr-ok::before {
    content: '';
    width: 7px; height: 7px;
    border-radius: 7px;
    background: var(--strom-ok);
    box-shadow: 0 0 0 3px rgba(44,153,96,0.15);
}
.status-row .sr-val {
    font-family: var(--strom-font-mono);
    color: var(--strom-fg);
...
.status-row .sr-err::before {
    content: '';
    width: 7px; height: 7px;
    border-radius: 7px;
    background: var(--strom-err);
}
...
.sidebar-info {
    margin-top: 12px;
    padding: 10px 12px;
    background: var(--strom-sunken);
    border-radius: 8px;
    font-size: 11px;
    color: var(--strom-fg-soft);
    line-height: 1.5;
}
.sidebar-info .si-mono {
    font-family: var(--strom-font-mono);
    color: var(--strom-fg-muted);
}
...
.step-pip.done .pip-circle {
    background: var(--strom-accent);
    color: #fff;
    border: 1px solid var(--strom-accent);
}
.step-pip.active .pip-circle {
    background: var(--strom-accent-soft);
    color: var(--strom-accent-ink);
    border: 1px solid var(--strom-accent);
}
.step-pip.pending .pip-circle {
    background: var(--strom-elev);
    color: var(--strom-fg-muted);
...
.step-line.done { background: var(--strom-accent); }
.step-line.pending { background: var(--strom-line); }

/* ── Stat cards (metrics) ── */
.stat-card {
    background: var(--strom-elev);
    border: 1px solid var(--strom-line);
    border-radius: var(--strom-radius);
    padding: 18px 20px;
}
...
.product-card .size-chip {
    background: var(--strom-sunken);
    border: 1px solid var(--strom-line);
    border-radius: var(--strom-radius-sm);
    padding: 5px 12px;
    font-size: 12px;
    font-family: var(--strom-font-mono);
    color: var(--strom-fg);
...
.product-card .size-chip .qty-badge {
    background: var(--strom-line);
    border-radius: 4px;
    padding: 1px 5px;
    margin-left: 6px;
    font-size: 10px;
...
.strom-tag.neutral { background: var(--strom-sunken); color: var(--strom-fg-soft); border: 1px solid var(--strom-line); }
.strom-tag.accent  { background: var(--strom-accent-soft); color: var(--strom-accent-ink); border: 1px solid transparent; }
...
.badge-warn {
    ...
    padding: 3px 8px;
    border-radius: 999px;
    background: var(--strom-warn-soft);
    color: var(--strom-warn);
    border: 1px solid transparent;
    margin: 2px;
    font-weight: 500;
}
...
.result-products { margin-top: 18px; }
.result-row {
    display: flex;
    align-items: center;
    gap: 10px;
...
.result-row .rr-link {
    margin-left: auto;
    color: var(--strom-accent);
    text-decoration: none;
    font-size: 12px;
    font-family: var(--strom-font-mono);
}
.result-row .rr-link:hover { text-decoration: underline; }

/* ── No-image placeholder ── */
.no-image-placeholder {
    background: var(--strom-sunken);
    border: 1.5px dashed var(--strom-line);
    border-radius: 12px;
    padding: 60px 20px;
    text-align: center;
}
.no-image-placeholder p {
    color: var(--strom-fg-muted) !important;
    font-size: 13px;
    margin: 0;
}
...
.img-gallery img:hover {
    border-color: var(--strom-accent);
}

/* ── Expander polish ── */
[data-testid="stExpander"] {
    border: 1px solid var(--strom-line) !important;
    border-radius: var(--strom-radius) !important;
...
.desc-preview {
    background: var(--strom-sunken);
    border: 1px solid var(--strom-line-soft);
    border-radius: 8px;
    padding: 16px;
    font-size: 13px;
    line-height: 1.6;
...
.desc-preview ul { padding-left: 18px; margin: 8px 0 0 0; }
.desc-preview li { margin-bottom: 2px; }

/* ── Streamlit native overrides ── */
.stButton > button[kind="primary"] {
    background: var(--strom-fg) !important;
    color: #fff !important;
    border: 1px solid var(--strom-fg) !important;
    border-radius: var(--strom-radius-sm) !important;
    font-family: var(--strom-font-ui) !important;
    font-weight: 500 !important;
    letter-spacing: -0.01em;
}
...
[data-testid="stFileUploader"] {
    border: 1.5px dashed var(--strom-line) !important;
    border-radius: 12px !important;
    background: var(--strom-elev) !important;
}
[data-testid="stFileUploader"]:hover {
    border-color: var(--strom-accent) !important;
    background: var(--strom-accent-soft) !important;
}
.stCaption { font-family: var(--strom-font-mono) !important; letter-spacing: 0.02em; }
.stDivider { border-color: var(--strom-line-soft) !important; }

/* ── Sidebar nav items ── */
.side-nav { display: flex; flex-direction: column; gap: 2px; margin: 14px 0; }
.side-nav-item {
    display: flex;
    align-items: center;
    gap: 10px;
...
.analyze-header .ah-left {
    display: flex;
    align-items: center;
    gap: 10px;
}
.analyze-header .ah-pulse {
    width: 8px; height: 8px;
    border-radius: 8px;
    background: var(--strom-accent);
    animation: strom-pulse 1.5s ease-in-out infinite;
}
@keyframes strom-pulse {
    0%, 100% { opacity: 1; box-shadow: 0 0 0 0 rgba(74,111,165,0.4); }
    50% { opacity: 0.7; box-shadow: 0 0 0 6px rgba(74,111,165,0); }
}
.analyze-header .ah-text {
    font-size: 14px;
    font-weight: 500;
    color: var(--strom-fg);
}
.analyze-header .ah-pct {
    font-family: var(--strom-font-mono);
    font-size: 12px;
    color: var(--strom-fg-muted);
}
...
.analyze-log-entries .log-line {
    display: flex;
    gap: 12px;
}
.analyze-log-entries .log-time {
    color: var(--strom-fg-muted);
    flex-shrink: 0;
}
.analyze-log-entries .log-msg-accent { color: var(--strom-accent-ink); }
.analyze-log-entries .log-msg-muted { color: var(--strom-fg-soft); }
.analyze-log-entries .log-msg-ok { color: var(--strom-ok); }
.analyze-log-entries .log-msg-warn { color: var(--strom-warn); }
.analyze-log-entries .log-msg-err { color: var(--strom-err); }

/* ── Review table (flat grid) ── */
.review-table {
    border: 1px solid var(--strom-line);
    border-radius: 10px;
    overflow: hidden;
...
.review-table-header {
    display: grid;
    grid-template-columns: 2.2fr 1.2fr 60px 60px 100px 100px 90px;
    gap: 0;
    padding: 10px 16px;
    background: var(--strom-sunken);
...
.review-table-row .rt-product {
    min-width: 0;
}
.review-table-row .rt-title {
    font-weight: 500;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
}
.review-table-row .rt-sku {
    font-size: 11px;
    font-family: var(--strom-font-mono);
    color: var(--strom-fg-muted);
    margin-top: 1px;
}
.review-table-row .rt-color {
    display: flex;
    align-items: center;
    gap: 8px;
    font-size: 12px;
...
.review-table-row .rt-mono {
    font-family: var(--strom-font-mono);
    font-size: 12px;
}
.review-table-row .rt-price {
    text-align: right;
    font-family: var(--strom-font-mono);
    font-size: 12px;
    color: var(--strom-fg-soft);
}
.review-table-row .rt-price-bold {
    text-align: right;
    font-family: var(--strom-font-mono);
    font-size: 12px;
    font-weight: 500;
...
.review-table-row .rt-status { text-align: right; }

/* ── Action bar at bottom of review ── */
.review-actions {
    display: flex;
    justify-content: space-between;
    align-items: center;
...
.file-task:last-child { border-bottom: none; }
.file-task .ft-icon {
...
.file-task .ft-icon span {
    font-size: 9px;
    font-family: var(--strom-font-mono);
    color: var(--strom-fg-muted);
    letter-spacing: 0.06em;
}
.file-task .ft-info { flex: 1; min-width: 0; }
.file-task .ft-name {
    font-size: 13px;
    font-weight: 500;
    overflow: hidden;
...
/* ── Tighten step indicator max-width ── */
.step-indicator { max-width: 720px; }
...
.sidebar-divider {
    height: 1px;
    background: var(--strom-line-soft);
    margin: 14px 0;
}
</style>
...
def render_step_indicator(current_step: str):
    """Render circle-pip step progress indicator (Variation A design)."""
    steps = [
        ("upload", "Upload faktura"),
        ("analyzing", "Analysér"),
        ("review", "Review"),
        ("done", "Færdig"),
    ]
    # Map internal states to display steps
    if current_step == "pushing":
        current_step = "review"  # show review as active during push
    ...
    for idx, (key, label) in enumerate(steps):
        if idx < current_idx:
            state = "done"
            circle = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="m5 12 5 5 9-12"/></svg>'
            ...
            state = "pending"
            circle = str(idx + 1)
        html += f'<div class="step-pip {state}"><div class="pip-circle">{circle}</div><span class="pip-label">{label}</span></div>'
        if idx < len(steps) - 1:
            line_state = "done" if idx < current_idx else "pending"
...
for key, default in {
    ...
    "push_results": [],
    "metafield_defs": [],
    "uploaded_files_data": [],  # store file bytes for analyzing step
    "analyze_log": [],  # log entries for analyzing step
}.items():
...
Beskrivelsen starter ALTID med "Denne/Disse [type]..." — ALDRIG med produktnavnet eller brandet.
...
- "Disse solbriller er designet med et acetat-stel og brune gradientglas. Stellet har en bred konstruktion med markante stænger. Glassene er monteret i et fuldt stel."

FORMLEN:
1. "Denne/Disse [type] er fremstillet i [materiale]." — brug det danske ord for produkttypen (t-shirt, hoodie, skjorte, bukser, solbriller osv.)
2. "Den/De er designet med [pasform/snit/detaljer]." — nævn fit, krave, hals, lukning
3. 1-2 sætninger mere om fysisk konstruktion (lommer, ærmer, søm, kanter, forstærkninger, stel, glas, stænger osv.)

VIGTIGT FOR SOLBRILLER/EYEWEAR:
- Brug ALTID "stel" (frame) — ALDRIG "stol" (chair). Et par solbriller har et STEL.
- Beskriv: stel-materiale (acetat, metal, titanium), stel-form (rund, firkantet, cat-eye, pilot, rektangulær), glas-type (gradient, spejl, polariseret, solid), stænger (brede, slanke, tynde), konstruktion.
- Varier beskrivelserne — brug IKKE samme skabelon for alle solbriller. Nævn specifikke detaljer der adskiller hvert par.
...
- VARIER sprogbrugen mellem produkter — brug IKKE samme sætningsstruktur for alle produkter i en batch
...
def _clean_description(text: str, vendor: str = "") -> str:
    ...
    # Only exception would be actual furniture products, which STRØM doesn't sell
...
def _get_fallback_description(type_da: str, color: str) -> str:
    ...
    fallbacks = {
        ...
        "Solbriller": "Disse solbriller er designet med et acetat-stel og tonede glas. Stellet har en klassisk konstruktion med brede stænger. Glassene er monteret i et fuldt stel med UV-beskyttelse.",
        "Hatte": "Denne hat er designet med en struktureret front og buet skygge. Justerbar lukning bagpå for optimal pasform. Seks-panel konstruktion med broderede ventilationshuller.",
        "Caps": "Denne cap er designet med en struktureret front og buet skygge. Justerbar lukning bagpå for optimal pasform. Seks-panel konstruktion med broderede ventilationshuller.",
        "Kasketter": "Denne kasket er designet med en struktureret front og buet skygge. Justerbar lukning bagpå for optimal pasform. Seks-panel konstruktion med broderede ventilationshuller.",
        "Huer": "Denne hue er strikket i en blød kvalitet med ribkant. Foldet kant giver mulighed for at justere pasformen. Tæt strik i mellemvægt holder godt på varmen.",
    }
...
def _filter_images_with_vision(image_urls: list[str], product_title: str, api_key: str) -> list[str]:
    ...
    # Ask Claude to classify each image — STRICT: no models/people allowed
    try:
        client = anthropic.Anthropic(api_key=api_key)
        prompt_text = (
            f"Product: {product_title}\n\n"
            f"I have {len(url_map)} images. Classify each image STRICTLY.\n\n"
            f"ACCEPT ONLY these image types:\n"
            f"- Product on white/plain/transparent background (packshot)\n"
            f"- Product flat lay (product laid flat, photographed from above)\n"
            f"- Ghost mannequin (invisible mannequin, product appears to float)\n"
            f"- Product close-up / detail shot (showing texture, stitching, hardware)\n"
            f"- Product on a simple display stand or hanger\n\n"
            f"REJECT any image where:\n"
            f"- A human model is wearing, holding, or touching the product\n"
            f"- Any body part is visible (face, hands, arms, legs, torso, feet, skin)\n"
            f"- The product is shown in a lifestyle/editorial context with people\n"
            f"- The image shows a runway/catwalk scene\n"
            f"- A mannequin head/face is visible (ghost mannequin torso WITHOUT head is OK)\n\n"
            f"Reply with ONLY a JSON array of image numbers that pass the ACCEPT criteria. "
            f"Example: [1, 3, 4]\n"
            f"If NO images pass, reply: []"
        )
...
def find_product_images_and_details(vendor: str, style_code: str, title: str, max_images: int = 5) -> dict:
    ...
    # Specific "comme des" sub-brands BEFORE generic CDG catch-all
        ...
        if "-" in style_code:
            ...
            # A.P.C. model numbers start with a letter followed by digits (M26388, H26123, etc.)
            for part in parts:
                if len(part) >= 4 and part[0].isalpha() and any(c.isdigit() for c in part[1:]):
                    apc_model = part
                    break
        apc_encoded = requests.utils.quote(apc_model)
        brand_search_url = f"https://www.apc.fr/en/search?q={apc_encoded}"
        brand_domain = "apc.fr"
    ...
    alt_skus = set()
    # Split by common separators and try each meaningful part
    if "-" in style_code:
        parts = style_code.split("-")
        for part in parts:
            if len(part) >= 4:  # Lowered from 5 to catch more model numbers
                alt_skus.add(part)
    # Remove dots/spaces/dashes
    alt_skus.add(style_code.replace("-", ""))
    alt_skus.add(style_code.replace(".", ""))
    # For A.P.C. style codes: also try without the color prefix
    # Format: COLORCODE-MODELNUM (e.g., COHBU-M26388 → M26388)
...
def _find_product_page_from_search(search_url: str, sku_lower: str, domain: str, headers: dict) -> str:
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            ...
            # Use the LONGEST meaningful part of the SKU as main identifier
            # (NOT always the first chunk — e.g., A.P.C. "cohbu-m26388" → "m26388" is the model)
            main_sku = sku_lower
            if sku_parts:
                # Pick the longest part with mixed alpha+digits (most likely to be the model number)
                candidates = [p for p in sku_parts if len(p) >= 4 and any(c.isdigit() for c in p)]
                if candidates:
                    main_sku = max(candidates, key=len)
                elif sku_parts[0] and len(sku_parts[0]) >= 3:
                    main_sku = sku_parts[0]
...
def _is_valid_product_image(url: str) -> bool:
    """Check if URL looks like a valid product image (not a logo, icon, placeholder, or model shot)."""
    if not url or len(url) < 15:
        return False
    url_lower = url.lower()
    # Skip non-product images (logos, icons, UI elements)
    ...
    for pattern in skip_patterns:
        if pattern in url_lower:
            return False
    # Skip images that are clearly model/lifestyle shots based on URL patterns
    model_patterns = ["model", "lookbook", "look-book", "editorial", "campaign",
                      "runway", "catwalk", "on-body", "onbody", "worn", "wearing",
                      "lifestyle", "outfit", "styled", "look_", "_look",
                      "on-model", "onmodel", "worn-by", "wornby"]
...
def push_product_to_shopify(
    ...
    if not sizes:
        ...
        # This prevents hard crashes and ensures product is still created
        total_qty = sum(v.get("quantity", 0) for v in product.get("variants", []))
        if total_qty == 0:
            total_qty = 1  # At least 1 item
...
with st.sidebar:
    # STRØM Wordmark
    st.markdown(
        '<div class="strom-wordmark">STRØM<span class="wm-dash">—</span><span class="wm-sub">Import</span>'
        '<span class="wm-tag">retail a/s · intern</span></div>',
        unsafe_allow_html=True,
    )

    # Nav items (Variation A sidebar)
    upload_icon = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M12 15V3"/><path d="m7 8 5-5 5 5"/><path d="M5 15v4a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-4"/></svg>'
    history_icon = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M14 3H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/><path d="M14 3v6h6"/></svg>'
    shop_icon = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>'
    gear_icon = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>'

    st.markdown(
        f'<nav class="side-nav">'
        f'<div class="side-nav-item active">{upload_icon} Ny import</div>'
        f'<div class="side-nav-item">{history_icon} Historik</div>'
        f'<div class="side-nav-item">{shop_icon} Shopify-kø</div>'
        f'<div class="side-nav-item">{gear_icon} Indstillinger</div>'
        f'</nav>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div style="font-size:10px;font-family:var(--strom-font-mono);text-transform:uppercase;'
        'letter-spacing:0.08em;color:var(--strom-fg-muted);margin:4px 0 8px;">Status</div>',
        unsafe_allow_html=True,
    )

    # API status rows + EUR rate in one block
    claude_html = '<span class="sr-ok">OK</span>' if api_ok else '<span class="sr-err">Mangler</span>'
    shopify_html = '<span class="sr-ok">OK</span>' if shopify_ok else '<span class="sr-err">Mangler</span>'
    st.markdown(
        f'<div class="status-row"><span class="sr-label">Claude Vision</span>{claude_html}</div>'
        f'<div class="status-row"><span class="sr-label">Shopify Admin</span>{shopify_html}</div>'
        f'<div class="status-row"><span class="sr-label">EUR → DKK</span><span class="sr-val">7,46</span></div>',
        unsafe_allow_html=True,
    )

    # Nationalbanken info box
    st.markdown(
        '<div class="sidebar-info">'
        'Kurs hentet fra Nationalbanken<br>'
        '<span class="si-mono">i dag · automatisk</span>'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    if shopify_ok:
        ...
        st.markdown(
            f'<div class="sidebar-info" style="margin-top:8px">'
            f'Store: <strong>{SHOPIFY_STORE}</strong><br>'
            f'<span class="si-mono">{token_preview} · {len(SHOPIFY_ACCESS_TOKEN)} tegn</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
    ...
    # EUR rate with status row style
    st.markdown(
        '<div style="font-size:10px;font-family:var(--strom-font-mono);text-transform:uppercase;'
        'letter-spacing:0.08em;color:var(--strom-fg-muted);margin:20px 0 6px;">Konfiguration</div>',
        unsafe_allow_html=True,
    )
...
if st.session_state.step == "upload":
    ...
    st.markdown('<div class="section-kicker">Trin 1 af 4</div>', unsafe_allow_html=True)
    st.markdown("## Upload faktura")
    st.markdown(
        '<div class="section-desc">Træk PDF-fakturaer hertil. Systemet udtrækker produkter, størrelser og priser automatisk med Claude Vision.</div>',
        unsafe_allow_html=True,
    )

    # Custom upload zone visual (sits above native uploader)
    upload_svg = '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><path d="M12 15V3"/><path d="m7 8 5-5 5 5"/><path d="M5 15v4a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-4"/></svg>'
    st.markdown(
        f'<div class="upload-zone">'
        f'<div class="uz-icon">{upload_svg}</div>'
        f'<div class="uz-title">Slip PDF-fakturaer her</div>'
        f'<div class="uz-sub">eller vælg fra computer · op til 50 MB</div>'
        f'<div class="uz-formats">PDF</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    if uploaded_files and st.button(f"Analysér {len(uploaded_files)} faktura{'er' if len(uploaded_files) > 1 else ''}", type="primary"):
        # Store file data and transition to analyzing step
        files_data = []
        for uf in uploaded_files:
            pdf_bytes = uf.read()
            files_data.append({"name": uf.name, "bytes": pdf_bytes, "size": len(pdf_bytes)})
...
# ═══════════════════════════════════════════════
    ...
    render_step_indicator("analyzing")
    st.markdown('<div class="section-kicker">Trin 2 af 4</div>', unsafe_allow_html=True)
    st.markdown("## Analysér")
    st.markdown(
        '<div class="section-desc">Claude Vision læser fakturaerne og udtrækker produktdata, størrelser og priser.</div>',
        unsafe_allow_html=True,
    )
    ...
    if not files_data:
        st.warning("Ingen filer at analysere.")
        st.session_state.step = "upload"
        st.rerun()

    # File task rows
    file_rows_html = '<div style="border:1px solid var(--strom-line);border-radius:10px;overflow:hidden;margin-bottom:20px">'
    for fd in files_data:
        size_str = f"{fd['size'] / 1024:.0f} KB" if fd['size'] < 1024*1024 else f"{fd['size'] / (1024*1024):.1f} MB"
        file_rows_html += (
            f'<div class="file-task">'
            f'<div class="ft-icon"><span>PDF</span></div>'
            f'<div class="ft-info"><div class="ft-name">{fd["name"]}</div><div class="ft-size">{size_str}</div></div>'
            f'<span class="strom-tag accent">Klar</span>'
            f'</div>'
        )
    file_rows_html += '</div>'
    st.markdown(file_rows_html, unsafe_allow_html=True)
    ...
    def _add_log(msg, tone="muted"):
        ts = _time.strftime("%H:%M:%S")
        log_entries.append({"t": ts, "m": msg, "tone": tone})
        _render_log(log_entries, log_container)

    def _render_log(entries, container):
        lines = ""
        for e in entries:
            css_class = f"log-msg-{e['tone']}"
            lines += f'<div class="log-line"><span class="log-time">{e["t"]}</span><span class="{css_class}">{e["m"]}</span></div>'
        container.markdown(
            f'<div class="analyze-log"><div class="al-label">Log</div>'
            f'<div class="analyze-log-entries">{lines}</div></div>',
            unsafe_allow_html=True,
        )
    ...
    for fi, fd in enumerate(files_data):
        file_pct_base = fi / total_files
        file_pct_step = 1.0 / total_files

        _add_log(f"Claude Vision · analyserer {fd['name']}", "accent")
        progress_bar.progress(file_pct_base + file_pct_step * 0.1)

        pdf_bytes = fd["bytes"]
        pdf_text = extract_pdf_text(pdf_bytes)
        pdf_images = extract_pdf_pages_as_images(pdf_bytes, dpi=150)
        _add_log(f"PDF konverteret: {len(pdf_images)} sider", "muted")
        progress_bar.progress(file_pct_base + file_pct_step * 0.2)

        # Pre-parse table structure
        table_products = parse_invoice_tables(pdf_bytes)
        if table_products:
            _add_log(f"Tabelparser fandt {len(table_products)} produktrækker", "muted")
            for tp in table_products:
                sizes_str = ", ".join([f"{v['size']}({v['quantity']})" for v in tp['variants']]) if tp['variants'] else f"Total: {tp['total_qty']}"
                _add_log(f"{tp['style_code']} — {tp['designation']} · {sizes_str}", "ok")
        else:
            _add_log("Ingen tabelstruktur — Vision udtrækker direkte", "warn")
        progress_bar.progress(file_pct_base + file_pct_step * 0.35)

        # Claude Vision extraction
        _add_log(f"Sender til Claude Vision...", "accent")
        file_products = []
        try:
            file_products = extract_products_with_ai(
            ...
            progress_bar.progress(file_pct_base + file_pct_step * 0.6)

            for p in file_products:
                variants = p.get("variants", [])
                total_qty = sum(v.get("quantity", 0) for v in variants)
                if len(variants) <= 1:
                    _add_log(f"ADVARSEL · {p.get('title', '?')}: Kun {len(variants)} størrelse(r)", "warn")
                p["product_type_da"] = map_type_danish(p.get("product_type", ""))
                p["variants"] = sort_sizes(p.get("variants", []))
                p["retail_price_dkk"] = calculate_retail_price(p.get("cost_price_eur", 0), eur_rate)
                p["cost_dkk"] = round(p.get("cost_price_eur", 0) * eur_rate, 2)
                p["computed_tags"] = build_tags(p)

            _add_log(f"{fd['name']}: {len(file_products)} produkter fundet", "ok")
            all_products.extend(file_products)
        except Exception as e:
            _add_log(f"FEJL · {fd['name']}: {str(e)}", "err")

        progress_bar.progress(file_pct_base + file_pct_step * 0.7)

        # Image search
        if file_products:
            _add_log("Søger produktbilleder og detaljer...", "accent")
        for p in file_products:
            ...
            if scraped.get("material") and not p.get("material"):
                ...
                p["material"] = mat_da
                _add_log(f"Materiale: {mat_da}", "muted")

            if scraped.get("description_en") and (not p.get("details_en") or len(p.get("details_en", "")) < 20):
                p["details_en"] = scraped["description_en"][:300]

        progress_bar.progress(file_pct_base + file_pct_step * 0.9)

    # Check for duplicate SKUs in Shopify
    shopify_ok = bool(SHOPIFY_ACCESS_TOKEN)
    if shopify_ok and all_products:
        _add_log("Tjekker for duplikater i Shopify...", "accent")
        ...
        for p in all_products:
            ...
            if sku:
                ...
                if existing:
                    ...
                    _add_log(f"ADVARSEL · {p['title']} (SKU: {sku}) — duplikat: {dupe_names}", "warn")

    progress_bar.progress(1.0)
    _add_log(f"Analyse afsluttet — {len(all_products)} produkter klar til review", "ok")

    if all_products:
        st.session_state.products = all_products
        import time as _t2
        _t2.sleep(1)  # Brief pause to let user see completion
        st.session_state.step = "review"
        st.rerun()
    else:
        st.error("Ingen produkter fundet. Prøv andre fakturaer.")
...
# ═══════════════════════════════════════════════
    ...
    render_step_indicator("review")
    products = st.session_state.products

    # ── Section header ──
    st.markdown('<div class="section-kicker">Trin 3 af 4</div>', unsafe_allow_html=True)

    # ── Dashboard stats ──
    total_products = len(products)
    total_items = sum(sum(v.get("quantity", 0) for v in p.get("variants", [])) for p in products)
    brands = list(set(p.get("vendor", "?") for p in products))
    prices = [p.get("retail_price_dkk", 0) for p in products if p.get("retail_price_dkk", 0) > 0]
    missing_images = sum(1 for p in products if not p.get("image_urls") and not p.get("image_url"))
    missing_material = sum(1 for p in products if not p.get("material") or p.get("material", "").lower() in ("ikke oplyst", "n/a", ""))
    has_dupes = sum(1 for p in products if p.get("_duplicate_warning"))
    issues_count = missing_images + missing_material + has_dupes

    selected_count = total_products
    st.markdown(f"## Review produkter")
    st.markdown(
        f'<div class="section-desc">{selected_count} af {total_products} produkter klar. '
        f'Ret priser, varianter eller SKU\'er før push.</div>',
        unsafe_allow_html=True,
    )

    s1, s2, s3, s4, s5 = st.columns(5)
    with s1:
        st.markdown(f'<div class="stat-card"><div class="stat-label">Produkter</div><div class="stat-value">{total_products}</div></div>', unsafe_allow_html=True)
    with s2:
        st.markdown(f'<div class="stat-card"><div class="stat-label">Enheder</div><div class="stat-value">{total_items}</div></div>', unsafe_allow_html=True)
    with s3:
        st.markdown(f'<div class="stat-card"><div class="stat-label">Brands</div><div class="stat-value">{len(brands)}</div></div>', unsafe_allow_html=True)
    with s4:
        price_range = f"{min(prices):.0f}–{max(prices):.0f}" if prices else "–"
        st.markdown(f'<div class="stat-card"><div class="stat-label">Prisrange DKK</div><div class="stat-value" style="font-size:1.3rem">{price_range}</div></div>', unsafe_allow_html=True)
    with s5:
        ok_color = "var(--strom-ok)" if issues_count == 0 else "var(--strom-warn)"
        st.markdown(f'<div class="stat-card"><div class="stat-label">Advarsler</div><div class="stat-value" style="color:{ok_color}">{issues_count}</div></div>', unsafe_allow_html=True)

    # Issue summary badges
    if issues_count > 0:
        badge_html = ""
        if missing_images > 0:
            badge_html += f'<span class="badge-warn">{missing_images} mangler billeder</span>'
        if missing_material > 0:
            badge_html += f'<span class="badge-warn">{missing_material} mangler materiale</span>'
        if has_dupes > 0:
            badge_html += f'<span class="badge-error">{has_dupes} mulige duplikater</span>'
        st.markdown(badge_html, unsafe_allow_html=True)
    else:
        st.markdown('<span class="badge-ok">Klar til import</span>', unsafe_allow_html=True)

    st.markdown("")

    # ── Color swatch map ──
    def _swatch_color(color_name):
        c = (color_name or "").lower()
        swatches = {
            "black": "#1a1a1a", "sort": "#1a1a1a", "noir": "#1a1a1a",
            "white": "#f5f5f5", "hvid": "#f5f5f5", "blanc": "#f5f5f5",
            "blue": "#4a6fa5", "blå": "#4a6fa5", "navy": "#2c3e6b",
            "red": "#c03030", "rød": "#c03030", "rouge": "#c03030",
            "green": "#2c9960", "grøn": "#2c9960", "vert": "#2c9960",
            "grey": "#8e92a2", "gray": "#8e92a2", "grå": "#8e92a2", "gris": "#8e92a2",
            "brown": "#8b6842", "brun": "#8b6842", "marron": "#8b6842",
            "beige": "#d4c5a9", "sand": "#d4c5a9", "sable": "#d4c5a9",
            "pink": "#d4739d", "rose": "#d4739d", "rosa": "#d4739d",
            "yellow": "#d4b830", "gul": "#d4b830", "jaune": "#d4b830",
            "orange": "#d48830", "purple": "#7a4a9e", "lilla": "#7a4a9e",
            "ivory": "#f5f0e0", "cream": "#f5eed6", "creme": "#f5eed6",
            "graphite": "#4a4a4a", "charcoal": "#3a3a3a",
            "cornflower": "#6495ed", "sage": "#88a480", "khaki": "#b8a97e",
            "olive": "#6b7a3d", "burgundy": "#6b1a2a", "bordeaux": "#6b1a2a",
            "camel": "#c4a46d", "tan": "#c4a46d", "taupe": "#b0a090",
        }
    ...
    # ── Flat review table (matching Variation A design) ──
    _rate = st.session_state.get("eur_rate", EUR_TO_DKK)

    # Build all variant rows for the flat table
    table_header = (
        '<div class="review-table"><div class="review-table-header">'
        '<span>Produkt / SKU</span>'
        '<span>Farve</span>'
        '<span>Str</span>'
        '<span style="text-align:right">Antal</span>'
        '<span style="text-align:right">EUR</span>'
        '<span style="text-align:right">DKK</span>'
    ...
    for i, p in enumerate(products):
        cost_eur = p.get("cost_price_eur", 0)
        retail_dkk = p.get("retail_price_dkk", 0)
        color = p.get("color", "–")
        ...
        if not p.get("image_urls") and not p.get("image_url"):
            p_issues.append("billeder")
        if not p.get("material") or p.get("material", "").lower() in ("ikke oplyst", "n/a", ""):
            p_issues.append("materiale")
        if p.get("_duplicate_warning"):
            p_issues.append("duplikat")
        if cost_eur == 0:
            p_issues.append("pris")

        if p.get("_duplicate_warning"):
            status_badge = '<span class="badge-error">Duplikat</span>'
        elif p_issues:
            status_badge = f'<span class="badge-warn">Advarsel</span>'
        else:
            status_badge = '<span class="badge-ok">OK</span>'

        # One row per variant (size), or one row if no variants
        if variants:
            for vi, v in enumerate(variants):
                size = v.get("size", "–")
                qty = v.get("quantity", 0)
                # Show product name only on first variant row
                if vi == 0:
                    product_cell = f'<div class="rt-product"><div class="rt-title">{title}</div><div class="rt-sku">{vendor} · {sku}</div></div>'
                    color_cell = f'<div class="rt-color"><span class="rt-swatch" style="background:{swatch}"></span>{color}</div>'
                    status_cell = f'<div class="rt-status">{status_badge}</div>'
                else:
                    product_cell = '<div class="rt-product"></div>'
                    color_cell = '<div></div>'
                    status_cell = '<div></div>'

                table_rows += (
                    f'<div class="review-table-row">'
                    f'{product_cell}'
                    f'{color_cell}'
                    f'<span class="rt-mono">{size}</span>'
                    f'<span class="rt-mono" style="text-align:right">{qty}</span>'
                    f'<span class="rt-price">{cost_eur:,.2f}</span>'
                    f'<span class="rt-price-bold">{retail_dkk:,.0f}</span>'
                    f'{status_cell}'
                    f'</div>'
                )
        else:
            ...
            table_rows += (
                f'<div class="review-table-row">'
                f'<div class="rt-product"><div class="rt-title">{title}</div><div class="rt-sku">{vendor} · {sku}</div></div>'
                f'<div class="rt-color"><span class="rt-swatch" style="background:{swatch}"></span>{color}</div>'
                f'<span class="rt-mono">–</span>'
                f'<span class="rt-mono" style="text-align:right">{total_qty}</span>'
                f'<span class="rt-price">{cost_eur:,.2f}</span>'
                f'<span class="rt-price-bold">{retail_dkk:,.0f}</span>'
                f'<div class="rt-status">{status_badge}</div>'
                f'</div>'
            )

    table_html = table_header + table_rows + '</div>'
    st.markdown(table_html, unsafe_allow_html=True)

    # ── Expandable detail panels per product (below table) ──
    st.markdown("")
    st.markdown(
        '<div style="font-size:11px;font-family:var(--strom-font-mono);text-transform:uppercase;'
        'letter-spacing:0.08em;color:var(--strom-fg-muted);margin:8px 0 12px;">Produktdetaljer</div>',
        unsafe_allow_html=True,
    )
    ...
    for i, p in enumerate(products):
        ...
        with st.expander(f"{p['vendor']} — {p['title']} ({total_qty} stk)"):
            # Editable fields
            ...
            with ec6:
                new_tags = st.text_input(
                    "Tags (kommasepareret)", value=tags_str, key=f"tags_{i}",
                )
            ...
            if img_urls:
                img_cols = st.columns(min(len(img_urls), 5))
                for ti, turl in enumerate(img_urls[:5]):
                    with img_cols[ti]:
                        st.image(turl, use_container_width=True)

    # ── Action bar (matching design: Tilbage | Gem kladde | Push X til Shopify) ──
    st.markdown("---")
    act_left, act_mid, act_right = st.columns([1, 1, 2])
    with act_left:
        if st.button("Tilbage", use_container_width=True):
            ...
            st.rerun()
    with act_right:
        can_push = bool(SHOPIFY_ACCESS_TOKEN) and bool(ANTHROPIC_API_KEY)
        if st.button(f"Push {total_products} til Shopify", type="primary", disabled=not can_push, use_container_width=True):
            st.session_state.step = "pushing"
            st.rerun()


# ═══════════════════════════════════════════════
# STEP 4: Push (transition to done)
# ═══════════════════════════════════════════════
elif st.session_state.step == "pushing":
    render_step_indicator("pushing")
    st.markdown('<div class="section-kicker">Trin 4 af 4</div>', unsafe_allow_html=True)
    st.markdown("## Opretter i Shopify")
    st.markdown(
        '<div class="section-desc">Produkter, varianter og lager synkroniseres til Shopify.</div>',
        unsafe_allow_html=True,
    )
...
# ═══════════════════════════════════════════════
    ...
    render_step_indicator("done")

    results = st.session_state.push_results
    ok = [r for r in results if r["status"] == "ok"]
    fail = [r for r in results if r["status"] == "error"]
    warnings_count = sum(len(r.get("warnings", [])) for r in ok)
    total_variants = sum(
        len(p.get("variants", [])) for p in st.session_state.products
    )

    # ── Result card (Variation A design) ──
    # Build product rows HTML
    rows_html = ""
    for r in ok:
        product_gid = r.get("id", "")
        numeric_id = product_gid.split("/")[-1] if "/" in product_gid else ""
        shopify_url = f"https://{SHOPIFY_STORE}.myshopify.com/admin/products/{numeric_id}" if numeric_id else ""

        if r.get("warnings"):
            badge = f'<span class="badge-warn">{len(r["warnings"])} advarsler</span>'
        else:
            badge = '<span class="badge-ok">OK</span>'

        link = f'<a class="rr-link" href="{shopify_url}" target="_blank">Shopify →</a>' if shopify_url else ""
        rows_html += f'<div class="result-row">{badge} <strong>{r["title"]}</strong>{link}</div>'

    for r in fail:
        rows_html += f'<div class="result-row"><span class="badge-error">Fejl</span> <strong>{r["title"]}</strong> <span style="color:var(--strom-fg-muted);font-size:12px">{r["error"]}</span></div>'

    icon = "✓" if not fail else "!"
    icon_bg = "var(--strom-ok-soft)" if not fail else "var(--strom-warn-soft)"
    icon_color = "var(--strom-ok)" if not fail else "var(--strom-warn)"
    title_text = f"{len(ok)} produkter oprettet i Shopify" if not fail else f"{len(ok)} oprettet, {len(fail)} fejlede"
    sub_text = "Varianter og lager er synkroniseret" if not fail else "Se detaljer nedenfor"

    result_html = f'''
    <div class="result-card">
        <div class="result-header">
            <div class="result-icon" style="background:{icon_bg};color:{icon_color}">{icon}</div>
            <div>
                <div class="rh-title">{title_text}</div>
                <div class="rh-sub">{sub_text}</div>
            </div>
        </div>
        <div class="result-metrics">
            <div class="result-metric"><div class="rm-label">Produkter</div><div class="rm-value">{len(ok)}</div></div>
            <div class="result-metric"><div class="rm-label">Varianter</div><div class="rm-value">{total_variants}</div></div>
            <div class="result-metric"><div class="rm-label">Advarsler</div><div class="rm-value" style="color:{icon_color}">{warnings_count}</div></div>
        </div>
        <div class="result-products">{rows_html}</div>
    </div>'''
    ...
    for r in ok:
        if r.get("warnings"):
            for w in r["warnings"]:
                st.caption(f"  · {r['title']}: {w}")

    st.markdown("")
    if st.button("Ny import", type="primary", use_container_width=True):
        st.session_state.products = []
        st.session_state.push_results = []
        st.session_state.step = "upload"
        st.rerun()
