...
- Titlen = Produktnavn + Farvenavn (præcis som det står på fakturaen)
- Format: "Produktnavn Farvenavn" (f.eks. "Classic Shirt Noir", "Slim Trouser Beige")
- Brug det PRÆCISE produktnavn fra fakturaen + den farve der står på fakturaen
- FJERN kun style-koder og artikelkoder
- Ingen CAPS LOCK
- Rene, læsbare produktnavne med farve
...
PRODUKTDETALJER (VIGTIGT — 2-4 SÆTNINGER, KUN FUNKTIONEL BESKRIVELSE):
- Skriv en KORT beskrivende tekst på 2-4 sætninger om produktet på dansk
- Beskriv KUN det FYSISKE produkt: pasform, snit, krave, ærmer, lukning, lommer, længde
- ALDRIG styling tips ("style den med..."), ALDRIG modetips ("perfekt til...")
- ALDRIG tillægsord: "elegant", "raffineret", "eksklusiv", "tidløs", "sofistikeret", "enkel", "afslappet"
- ALDRIG subjektive vurderinger om brandet eller stilen
- ALDRIG skriv at du ikke kan udlede noget — skriv KUN det du VED
- ALDRIG nævn sæson/kollektion i detaljer — det har sit eget felt
- Hvis du ikke har nok info, skriv en KORT generisk fysisk beskrivelse baseret på produkttypen
- Gem dansk version i "details" og engelsk version i "details_en"
- GODT eksempel: "Løstsiddende bukser med lige ben og mellemhøj talje. Elastik i taljen og sidelommer."
- DÅRLIGT eksempel: "Perfekt til en afslappet hverdagslook. Typisk for mærkets enkle linje. Del af SS26-kollektionen."
...
[
  {{
    ...
    "title": "Produktnavn Farvenavn (fra faktura)",
    "vendor": "Brand Name",
...
def _get_all_images_from_product_page(product_url: str, sku_lower: str, headers: dict, max_images: int = 5) -> list[str]:
    """
    From a specific product page, extract ALL product images (up to max_images).
    Uses structured data (JSON-LD) first for multiple images, then OG, then img tags.
    """
    from bs4 import BeautifulSoup
    from urllib.parse import urljoin
    ...
    def _add_image(url: str):
        if not url or url in seen_urls or len(found_images) >= max_images:
            return
        if url.startswith("//"):
            url = "https:" + url
    ...
    try:
        ...
        # Method 1: JSON-LD structured data — can contain multiple images
        for script in soup.find_all("script", type="application/ld+json"):
            try:
                import json as _json
                ld = _json.loads(script.string or "")
        ...
        # Method 3: All og:image tags (some sites have multiple)
        for og in soup.find_all("meta", property="og:image"):
            if og.get("content"):
                _add_image(og["content"])

        # Method 4: Product images from img tags with SKU match or product-like URLs
...
def push_product_to_shopify(
    ...
    _add_mf(
        ...
        search_keys=["custom.brand_collection"],
        value=vendor,
    )
    ...
    if color:
        _add_mf(
            ...
            search_keys=["custom.color_name", "custom.color"],
            value=color,
        )

    # Material (not a standard metafield but may exist)
    if material:
        _add_mf(
            search_names=["material", "materiale"],
            search_keys=["custom.material"],
            value=material,
        )
