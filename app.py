...
def build_description_da(product: dict) -> str:
    """Build STRØM product description in Danish — functional, not salesy."""
    ...
    # Product details — descriptive, not adjective-heavy
    if details:
        lines.append(f"<p>{details}</p>")

    # Color + Material (material under color, in %)
    spec_lines = []
    if color:
        spec_lines.append(f"Farve: {color}")
    if material:
        spec_lines.append(f"Materiale: {material}")
    if country_name:
        spec_lines.append(f"Produceret i {country_name}")
    if spec_lines:
        lines.append("<p>" + "<br>".join(spec_lines) + "</p>")

    return "\n".join(lines)


def build_description_en(product: dict) -> str:
    """Build English translation of description — functional, not salesy."""
    title = product.get("title", "")
    vendor = product.get("vendor", "")
    color = product.get("color", "")
    material = product.get("material", "")
    ...
    # Simple material translation DA→EN
    material_en = material
    if material:
        material_en = (material
            .replace("bomuld", "cotton")
            .replace("uld", "wool")
            .replace("silke", "silk")
            .replace("hør", "linen")
            .replace("polyamid", "polyamide")
            .replace("viskose", "viscose")
            .replace("elastan", "elastane")
            .replace("kashmir", "cashmere")
        )

    lines = [f"<p>{title} from {vendor}.</p>"]

    # Details (keep Danish details for now — could be translated via AI later)
    if details_da:
        lines.append(f"<p>{details_da}</p>")

    spec_lines = []
    if color:
        spec_lines.append(f"Color: {color}")
    if material_en:
        spec_lines.append(f"Material: {material_en}")
    if country_name:
        spec_lines.append(f"Made in {country_name}")
    if spec_lines:
        lines.append("<p>" + "<br>".join(spec_lines) + "</p>")
...
MATERIALE-REGLER:
- Oversæt til dansk og angiv i procent: "100% COTON" → "100% bomuld", "70% LAINE 30% POLYESTER" → "70% uld, 30% polyester"
- COTON → bomuld, LAINE → uld, SOIE → silke, LIN → hør, POLYAMIDE → polyamid, VISCOSE → viskose, ELASTHANNE → elastan, CACHEMIRE → kashmir

PRODUKTDETALJER (VIGTIGT):
- Udtræk konkrete, beskrivende detaljer om produktet — IKKE tillægsord som "elegant" eller "raffineret"
- Beskriv: ærmelængde (kort/lang), kravetype (rund hals/V-hals/skjortekrave), pasform (regular/slim/oversized/loose), lukning (knapper/lynlås/uden), lommer, længde, specielle detaljer
- Basér det på produktnavn, type og hvad der kan udledes fra fakturaen
- Gem i feltet "details" som en kort dansk beskrivelse
...
[
  {{
    ...
    "material": "dansk materiale i % (f.eks. 100% bomuld)",
    "details": "kort beskrivelse af snit/pasform/detaljer på dansk",
...
def push_product_to_shopify(
    ...
    # ── 4. Set metafields (gender ALWAYS set + brand = vendor) ──
    gender = product.get("gender", "").lower()
    gender_values = []
    if gender == "unisex":
        # Unisex → both checkboxes checked
        ...
        # Fallback: always set gender — default to Women if unknown
        gender_values = ["Women"]

    metafields = []

    # Gender — ALWAYS set, never empty
    ...
    # Brand collection = vendor (from Product Organization)
    metafields.append({
        "namespace": "custom",
