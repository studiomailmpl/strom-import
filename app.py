...
STØRRELSER + ANTAL (MEST KRITISKE FELT — FEJL HER ER IKKE ACCEPTABELT):
- Udtræk ALLE størrelser og det PRÆCISE antal pr. størrelse fra HVER linje i fakturaen
- Hvert produkt SKAL have ALLE varianter der står i fakturaen — IKKE kun den første
- Typisk format i fakturaer: kolonner med S, M, L, XL (eller 36, 38, 40, 42) og et tal under hver
- Hvis en størrelse har antal 0, medtag den IKKE
- Hvis en størrelse har antal >= 1, SKAL den med
- Eksempel: Hvis fakturaen viser "XS:0, S:2, M:3, L:2, XL:1" → variants: [S(2), M(3), L(2), XL(1)]
- Det SAMLEDE antal stk skal stemme med fakturalinjens totale antal
- ALDRIG returner kun én størrelse hvis fakturaen viser flere
  ...
  {{
    ...
    message = client.messages.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"Udtræk alle produkter fra denne leverandørfaktura. VIGTIGT: Hver produkt SKAL have ALLE størrelser med det præcise antal. Tjek HVER linje i fakturaen for størrelsesfordelingen.\n\n{pdf_text}",
            }
        ],
...
def push_product_to_shopify(
    ...
    # Find the matching collection GID for the vendor
    ...
    for col in collections:
        ...
        if vendor_lower_match in col_title or vendor_lower_match in col_handle:
