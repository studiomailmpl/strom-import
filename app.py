...
def _get_all_images_from_product_page(product_url: str, sku_lower: str, headers: dict, max_images: int = 5) -> list[str]:
    ...
    # Content-based deduplication: download and hash to find truly unique images
    unique_urls = []
    seen_hashes = set()
    for url in candidates:
        ...
        try:
            ...
            if not resp.ok or len(resp.content) < 1000:
                continue
            # Hash first 50KB of image content (enough to detect duplicates, fast)
            content_hash = hashlib.md5(resp.content[:50000]).hexdigest()
            if content_hash in seen_hashes:
                continue  # Duplicate image — skip
            seen_hashes.add(content_hash)
            unique_urls.append(url)
        except Exception:
            # If we can't download to verify, include it (Shopify will try later)
            unique_urls.append(url)
