...
def extract_products_with_ai(pdf_text: str, existing_tags: list[str], pdf_images: list[str] = None) -> list[dict]:
...
[
  {{
    ...
    # Build message content — use Vision (images) + text for best table extraction
    user_content = []

    # Add PDF page images FIRST so the AI can visually read the table layout
    if pdf_images:
        user_content.append({
            "type": "text",
            "text": "Her er faktura-siderne som billeder. KIG PÅ TABELLEN og aflæs ALLE størrelser og antal for hvert produkt:",
        })
    ...
    # Add text extraction as supplement
    user_content.append({
        "type": "text",
        "text": f"""Udtræk alle produkter fra denne leverandørfaktura.

KRITISK INSTRUKTION FOR STØRRELSER:
1. Brug BILLEDERNE ovenfor til at aflæse tabellen visuelt — tekst-udtræk herunder kan mangle tabelstruktur
2. Find tabel-headeren med størrelses-kolonnerne (f.eks. XS, S, M, L, XL eller 36, 38, 40, 42)
3. For HVER produktlinje, aflæs antallet under HVER størrelses-kolonne
4. Hvert produkt har typisk 3-7 størrelser — hvis du kun finder 1, har du lavet en fejl
5. Dobbelttjek at summen af alle størrelsers antal matcher linjens total
...
if st.session_state.step == "upload":
    ...
    if uploaded_files and st.button("Udtræk produkter", type="primary"):
        ...
        for uploaded_file in uploaded_files:
            ...
            with st.spinner(f"Konverterer PDF-sider til billeder..."):
                pdf_images = extract_pdf_pages_as_images(pdf_bytes, dpi=200)
                st.caption(f"  {len(pdf_images)} sider konverteret til billeder for Vision API")

            with st.spinner(f"AI udtræk fra {uploaded_file.name} (Vision + tekst)..."):
                try:
                    products = extract_products_with_ai(
                        pdf_text, st.session_state.existing_tags, pdf_images=pdf_images
                    )
                    for p in products:
