...
# ═══════════════════════════════════════════════
    ...
    for i, p in enumerate(products):
        ...
        with st.expander(
            ...
            if p.get("_duplicate_warning"):
                ...
                st.error(f"⚠️ DUPLIKAT: {p.get('style_code', '')} — {dupe_list}")
            if p.get("cost_price_eur", 0) == 0:
                st.error("Kostpris er 0 — produktet kan ikke prissættes")
            if p.get("retail_price_dkk", 0) > 15000:
                st.warning(f"Høj pris: {p.get('retail_price_dkk', 0):.0f} DKK")
            ...
            with preview_col:
                ...
                if img_urls:
                    ...
                    # Thumbnail strip
                    if len(img_urls) > 1:
                        thumb_cols = st.columns(min(len(img_urls), 5))
                    ...
                    st.markdown(
                        ...
                        '<p>Ingen billeder fundet</p></div>',
                        unsafe_allow_html=True
                    )

                # Product card preview
                size_chips_html = "".join([
                    f'<span class="size-chip">{v["size"]}<span class="qty-badge">{v["quantity"]}</span></span>'
                    for v in p.get("variants", [])
                ])
                st.markdown(
                    f'''<div class="product-card">
                        <p class="brand-label">{p["vendor"].upper()}</p>
                        <p class="product-title">{p["title"]}</p>
                        <p class="product-color">{p.get("color", "")}</p>
                        <p class="product-price">{p.get("retail_price_dkk", 0):.0f} DKK</p>
                        <div class="size-chips">{size_chips_html}</div>
                    </div>''',
                    unsafe_allow_html=True,
                )
            ...
            with data_col:
                # ── Shopify data fields ──
                mono = "font-family:var(--strom-font-mono);font-size:12px"
                fields_html = f'''
                <div>
                    <div class="data-row"><span class="data-label">SKU</span><span class="data-value" style="{mono}">{p.get("style_code", "–")}</span></div>
                    <div class="data-row"><span class="data-label">Type</span><span class="data-value">{p.get("product_type_da", "–")}</span></div>
                    <div class="data-row"><span class="data-label">Vendor</span><span class="data-value">{p["vendor"]}</span></div>
                    <div class="data-row"><span class="data-label">Køn</span><span class="data-value">{p.get("gender", "–")}</span></div>
                    <div class="data-row"><span class="data-label">Farve</span><span class="data-value">{p.get("color", "–")}</span></div>
                    <div class="data-row"><span class="data-label">Materiale</span><span class="data-value">{p.get("material", "–") or "–"}</span></div>
                    <div class="data-row"><span class="data-label">Cost</span><span class="data-value" style="{mono}">{cost_dkk:.2f} DKK (€{p.get("cost_price_eur", 0):.2f})</span></div>
                    <div class="data-row"><span class="data-label">Retail</span><span class="data-value" style="font-weight:600;font-family:var(--strom-font-display);font-size:15px">{p.get("retail_price_dkk", 0):.0f} DKK</span></div>
                    <div class="data-row"><span class="data-label">Sæson</span><span class="data-value">{p.get("season", "–")}</span></div>
                    <div class="data-row"><span class="data-label">Oprindelse</span><span class="data-value">{p.get("country_of_origin", "–")}</span></div>
                    <div class="data-row"><span class="data-label">Handle</span><span class="data-value" style="{mono};font-size:11px">{make_handle(p["vendor"], p["title"])}</span></div>
                </div>'''
                ...
                desc_html = build_description_da(p)
                st.markdown(f'<div class="desc-preview">{desc_html}</div>', unsafe_allow_html=True)

            # ── Editable fields — grouped in sections ──
            st.markdown("---")
            st.markdown("**Rediger felter**")
    ...
    with btn1:
        ...
        if st.button("Opret i Shopify", type="primary", disabled=not can_push, use_container_width=True):
            st.session_state.step = "pushing"
            st.rerun()
    with btn2:
        st.caption(f"{total_products} produkter — {total_items} enheder")
    with btn3:
