...
- Titlen skal KUN være produktnavnet — UDEN farve
- Format: "Produktnavn" (f.eks. "Classic Shirt", "Slim Trouser", "Oversized Knit")
- Farven skal ALDRIG stå i titlen — den angives separat i "color" feltet
- FJERN alle style-koder, artikelkoder, interne referencer
- FJERN farvekoder og farvenavne fra titlen
- Ingen bindestreger, ingen CAPS LOCK, ingen koder
- Rene, læsbare produktnavne uden farve
...
[
  {{
    ...
    "title": "Rent Produktnavn UDEN farve",
    "vendor": "Brand Name",
...
def find_product_image(vendor: str, style_code: str, title: str) -> str:
    """
    Search for product image using a two-step approach:
    1. Find the actual product PAGE (not just any image on a search results page)
    2. Get the main product image from that specific page

    This avoids mismatching images from search results.
    ...
    # ── Strategy 1: Brand website — find product page by SKU, then get image ──
    brand_search_url = ""
    brand_domain = ""
    ...
    if brand_search_url:
        ...
        if product_page_url:
            img = _get_main_image_from_product_page(product_page_url, sku_lower, headers)
            if img:
                return img
    ...
    # ── Strategy 3: Direct brand CDN patterns (some brands have predictable image URLs) ──
    if "american vintage" in vendor_lower:
        # American Vintage uses style code in their CDN URLs
        # Try common patterns
...
def _get_main_image_from_product_page(product_url: str, sku_lower: str, headers: dict) -> str:
    """
    From a specific product page, extract the main product image.
    Uses structured data (JSON-LD, Open Graph) first, then falls back to img tags.
    This is much more reliable than scraping search results.
    """
    from bs4 import BeautifulSoup
    from urllib.parse import urljoin
    ...
    try:
        ...
        # Method 1: Open Graph image (og:image) — most reliable, always the main product image
        ...
        # Method 2: JSON-LD structured data — second most reliable
        for script in soup.find_all("script", type="application/ld+json"):
            try:
                import json as _json
                ld = _json.loads(script.string or "")
        ...
        # Method 3: Twitter card image
        twitter_img = soup.find("meta", attrs={"name": "twitter:image"}) or soup.find("meta", attrs={"property": "twitter:image"})
        ...
        # Method 4: First large product image with SKU match in URL or alt
...
def _is_valid_product_image(url: str) -> bool:
    ...
    for pattern in skip_patterns:
        if pattern in url_lower:
            return False
    # Must end with an image extension or be from a CDN
...
def push_product_to_shopify(
    ...
    for edge in variant_edges:
        ...
        # Update inventory item: SKU, cost, weight, country, HS code
        try:
            shopify.update_inventory_item(
                ...
                hs_code=hs_code,
                weight_grams=float(retail_price),
                tracked=True,
            )
        except Exception as e:
