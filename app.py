...
def build_description_da(product: dict) -> str:
    """Build STRØM product description in Danish — 3-5 lines, functional, with material."""
    ...
    # Line 1: Title + brand intro
    lines.append(f"<p>{title} fra {vendor}.</p>")

    # Line 2-3: Product details — descriptive (sleeve, fit, neckline, etc.)
    if details:
        lines.append(f"<p>{details}</p>")
    else:
        # Fallback if AI didn't provide details
        lines.append(f"<p>{type_da} fra {vendor} i {color}.</p>" if color else f"<p>{type_da} fra {vendor}.</p>")

    # Line 4: Color + Material (ALWAYS include material if available)
    spec_parts = []
    if color:
        spec_parts.append(f"Farve: {color}")
    if material:
        spec_parts.append(f"Materiale: {material}")
    if spec_parts:
        lines.append("<p>" + "<br>".join(spec_parts) + "</p>")

    # Line 5: Origin
    if country_name:
        lines.append(f"<p>Produceret i {country_name}.</p>")

    return "\n".join(lines)


def build_description_en(product: dict) -> str:
    """Build English translation of description — 3-5 lines, with material."""
    title = product.get("title", "")
    vendor = product.get("vendor", "")
    color = product.get("color", "")
    material = product.get("material", "")
    ...
    # Material translation DA→EN
    material_en = material
    if material:
        material_en = (material
            .replace("bomuld", "cotton").replace("uld", "wool")
            .replace("silke", "silk").replace("hør", "linen")
            .replace("polyamid", "polyamide").replace("viskose", "viscose")
            .replace("elastan", "elastane").replace("kashmir", "cashmere")
        )

    lines = []
    lines.append(f"<p>{title} from {vendor}.</p>")

    # Use English details if provided by AI, otherwise Danish
    detail_text = details_en or details_da
    if detail_text:
        lines.append(f"<p>{detail_text}</p>")

    spec_parts = []
    if color:
        spec_parts.append(f"Color: {color}")
    if material_en:
        spec_parts.append(f"Material: {material_en}")
    if spec_parts:
        lines.append("<p>" + "<br>".join(spec_parts) + "</p>")

    if country_name:
        lines.append(f"<p>Made in {country_name}.</p>")
...
FARVE-REGLER (KRITISK):
- Brug SIMPLE, genkendelige farvenavne som en forbruger forstår
- ALDRIG leverandørens interne farvenavne direkte
- Oversæt ALTID til simple farver:
...
PRODUKTDETALJER (VIGTIGT — 3-5 SÆTNINGER):
- Skriv en beskrivende tekst på 3-5 sætninger om produktet på dansk
- IKKE tillægsord som "elegant", "raffineret", "eksklusiv" — kun KONKRETE beskrivelser
- Beskriv: ærmelængde (kort/lang), kravetype (rund hals/V-hals/skjortekrave), pasform (regular/slim/oversized/loose), lukning (knapper/lynlås/uden), lommer, længde, kanter, detaljer
- Basér det på produktnavn, type og hvad der kan udledes fra fakturaen
- Gem dansk version i "details" og engelsk version i "details_en"
- Eksempel: "Løstsiddende bukser med lige ben og mellemhøj talje. Elastik i taljen og sidelommer. Længde til anklen."
...
STØRRELSER + ANTAL (KRITISK — FEJL HER ER IKKE ACCEPTABELT):
- Udtræk ALLE størrelser og det PRÆCISE antal pr. størrelse fra fakturaen
- Hvert produkt SKAL have mindst én variant med størrelse og antal
- Hvis der står "S:2, M:3, L:1" på fakturaen, skal det afspejles 1:1
- Spring ALDRIG størrelser eller antal over
...
def find_product_image(vendor: str, style_code: str, title: str) -> str:
    """
    Search for product packshot image using SKU/style_code as primary search term.
    Tries brand website first, then multi-brand retailers.
    ...
    # Build search URLs — SKU-based search on brand sites + multi-brand retailers
    search_urls = []

    # Brand-specific sites (search by SKU)
    if "american vintage" in vendor_lower:
        search_urls.append(f"https://www.americanvintage-store.com/en/search?q={sku_encoded}")
    elif "comme des" in vendor_lower or "cdg" in vendor_lower:
        search_urls.append(f"https://shop.doverstreetmarket.com/search?q={sku_encoded}")
    elif "acne" in vendor_lower:
        search_urls.append(f"https://www.acnestudios.com/dk/en/search?q={sku_encoded}")
    elif "norse projects" in vendor_lower:
        search_urls.append(f"https://www.norseprojects.com/search?q={sku_encoded}")
    elif "our legacy" in vendor_lower:
        search_urls.append(f"https://www.ourlegacy.com/search?q={sku_encoded}")

    # Multi-brand retailers (search by SKU, then vendor+SKU)
    search_urls.extend([
        f"https://www.ssense.com/en-dk/search?q={sku_encoded}",
        f"https://www.farfetch.com/dk/shopping/search/items/?q={sku_encoded}",
        f"https://www.mrporter.com/en-dk/search?query={sku_encoded}",
        f"https://www.matchesfashion.com/search?q={sku_encoded}",
    ])
    ...
    for url in search_urls:
        try:
            ...
            # Look for product images — prioritize packshots (no model)
            for img in soup.find_all("img"):
                ...
                is_product = any(kw in src_lower for kw in [
                    "product", "catalog", "media", "cdn.shopify",
                    "images/products", "cdn-images", "img.ssense",
                    "cdn.shopify.com", "imgix", "cloudfront",
                ])
                has_sku_match = style_code.lower() in src_lower or style_code.lower() in alt

                if is_product or has_sku_match:
                    if src.startswith("//"):
                        src = "https:" + src
                    elif src.startswith("/"):
...
def push_product_to_shopify(
    ...
    for v in product.get("variants", []):
        ...
        variant_updates.append({
            ...
            "sku": f"{style_code}-{size}" if style_code else size,
    ...
    if variant_updates:
        try:
            ...
            errors_log.append(f"Variant update: {e}")
    ...
    # Match variants to original sizes by order
    original_variants = product.get("variants", [])
    size_to_qty = {v["size"]: v.get("quantity", 0) for v in original_variants}

    for var_node in final_variants:
        inv_item_id = var_node.get("inventoryItem", {}).get("id", "")
        if not inv_item_id:
            continue

        # Find the matching size for this variant
        var_size = ""
        if "selectedOptions" in var_node:
            for opt in var_node.get("selectedOptions", []):
                if opt["name"] == "Størrelse":
                    var_size = opt["value"]
        elif "title" in var_node:
            var_size = var_node["title"]

        # Set quantity using size mapping
        ...
        if country or hs_code:
            try:
                ...
                errors_log.append(f"Inventory cost ({var_size}): {e}")

    # ── 4. Set metafields (gender ALWAYS + brand_collection = vendor) ──
    gender = product.get("gender", "").lower()
    gender_values = []
    if gender == "unisex":
    ...
    # Gender — ALWAYS as list type for consistency
    metafields.append({
        "namespace": "custom",
        "key": "gender",
    ...
    try:
        ...
        errors_log.append(f"Metafields: {e}")
    ...
    if image_url:
        try:
            ...
            errors_log.append(f"Image: {e}")

    # ── 6. Publish to specific catalogs: Danmark, stromstore.com, stromstore.us ──
    REQUIRED_CHANNELS = ["danmark", "denmark", "stromstore.com", "stromstore.us", "online store", "point of sale"]
    if publications:
        pub_ids = []
        for p in publications:
            name_lower = p["name"].lower()
            # Always publish to all available channels, but ensure required ones are included
            pub_ids.append(p["id"])
        if pub_ids:
            try:
                shopify.publish_product(product_id, pub_ids)
            except Exception as e:
                errors_log.append(f"Publishing: {e}")
    ...
    for col in collections:
        ...
        if vendor_lower in col_title_lower or vendor_lower in col_handle_lower:
            try:
                ...
                errors_log.append(f"Collection ({col['title']}): {e}")
            break
    ...
    for i, p in enumerate(products):
        ...
        with st.expander(
            ...
            with data_col:
                ...
                st.text(f"Farve:     {p.get('color', 'N/A')}")
