...
def _get_all_images_from_product_page(product_url: str, sku_lower: str, headers: dict, max_images: int = 5) -> list[str]:
    ...
    def _dedup_key(url: str) -> str:
        ...
        if url.startswith("//"):
            url = "https:" + url
        parsed = urlparse(url)
        # Use only the path, stripped of ALL query params
        path = parsed.path.lower().strip("/")
        # Strip common CDN size/transform suffixes from the path
        # Shopify: _800x800, _1200x, _grande, _large, _medium, _small, _compact
        path = re.sub(r'_\d+x\d*(?=\.\w+$)', '', path)
        path = re.sub(r'_(grande|large|medium|small|compact|master|pico|icon|thumb)(?=\.\w+$)', '', path)
        # Generic CDN patterns: /w_800/, /h_600/, /c_fill/, /q_80/
        path = re.sub(r'/[whcq]_\d+', '', path)
        # Strip trailing image format variations
    ...
    # Content-based deduplication: download, resize to tiny thumbnail, then hash
    # This catches same-image-different-size duplicates
    unique_urls = []
    seen_hashes = set()
    ...
    for url in candidates:
        ...
        try:
            ...
            # Try perceptual dedup: resize to 16x16 and hash
            try:
                ...
                # Fallback: hash raw bytes if PIL unavailable
                content_hash = hashlib.md5(resp.content).hexdigest()

            if content_hash in seen_hashes:
                continue  # Duplicate image — skip
            seen_hashes.add(content_hash)
            unique_urls.append(url)
        except Exception:
            # Can't download to verify — skip rather than risk duplicate
            continue
...
def push_product_to_shopify(
    ...
    for edge in variant_edges:
        ...
        # Update inventory item: SKU, cost, country, HS code
        try:
