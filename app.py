...
def build_tags(product: dict) -> list[str]:
    ...
    # Filter out tags we already handle, English types, and gender variants
    skip_tags = {
        # English product types (we use Danish versions)
        ...
        "Womens", "Mens", "Womenswear", "Menswear", "Women", "Men",
        "Herrer", "Damer", "Unisex",
        # Generic tags that add no value (Tøj is added from product type, not AI)
        "Clothing", "Fashion", "New", "Sale",
    }
...
def extract_products_with_ai(pdf_text: str, existing_tags: list[str], pdf_images: list[str] = None, table_products: list[dict] = None, active_descriptions: list[dict] = None) -> list[dict]:
    ...
    - Description style/length comes from real active product examples
    """
    ...
    if active_descriptions:
        ...
        if good_examples:
            ...
            description_examples += "\nSkriv beskrivelser der matcher dette niveau af detalje og denne tone. Beskriv pasform, materialer, detaljer og konstruktion i 3-5 sætninger.\n"
...
PRODUKTDETALJER (3-5 SÆTNINGER, DETALJERET FYSISK BESKRIVELSE):
- Beskriv: pasform, snit, krave, ærmer, lukning, lommer, mønster, finish, konstruktionsdetaljer
- Skriv i en flydende, professionel tone som en premium modebutik
- FORBUDTE ORD: "fremgår ikke", "kan ikke udledes", "perfekt til", "ideel til",
  "elegant", "raffineret", "tidløs", "typisk for mærket", "kollektionen"
- Beskriv produktet konkret og detaljeret — ikke generisk
- Inkluder gerne: sømdetaljer, kantafslutninger, fit (slim/regular/relaxed/oversized), længde, talje-type, knap-stil
...
[
  {{
    ...
    "details": "3-5 sætninger dansk med detaljeret fysisk beskrivelse af pasform, snit, konstruktion og detaljer",
    "details_en": "3-5 sentences English with detailed physical description of fit, cut, construction and details",
