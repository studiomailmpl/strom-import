...
class ShopifyGraphQL:
    ...
    def search_products_by_sku(self, sku: str) -> list[dict]:
        """Search for existing products that contain this SKU in their variant SKUs.
        Returns list of matching products with id, title, status."""
        ...
        try:
            ...
            for edge in data.get("products", {}).get("edges", []):
                node = edge["node"]
                # Verify SKU actually matches (Shopify search can be fuzzy)
                variant_skus = [
                    ve["node"].get("sku", "")
                    for ve in node.get("variants", {}).get("edges", [])
                ]
                sku_lower = sku.lower()
                has_match = any(sku_lower in (vs or "").lower() for vs in variant_skus)
                if has_match:
                    results.append({
                        "id": node["id"],
                        "title": node["title"],
    ...
    def fetch_active_products(self, limit: int = 50) -> list[dict]:
        """Fetch active products with their descriptions, types, and vendors."""
        query = """
        query fetchActiveProducts($first: Int!, $query: String) {
        ...
        """
        data = self._execute(query, {"first": limit, "query": "status:active"})
        products = []
        for edge in data.get("products", {}).get("edges", []):
            node = edge["node"]
            products.append({
